...
Public Class InstanceGetter
	...
	Public Async Function [Get](ByVal param As GetInstanceRequest) As Task
		...
		If IsSuccessfulGetInstance Then
			...
			For Each instanceDocument As XElement In ins
				...
				Dim xbrlEndTime As Date
				If Date.TryParseExact(period, GetInstanceRequest.DateFormat, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, xbrlEndTime) Then
			...
			Dim instance As FileStream = New FileStream(instanceFileName, FileMode.Create)
			writer.Write(wf, instance)
			instance.Dispose()
	...
	Private Function PrepareMsg(ByVal responseErrorMessage As String) As String
		...
		Dim errorPhrases(3) As (Phrase As String, Code As Integer)
